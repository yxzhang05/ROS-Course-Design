...
{
  ros::init(argc, argv, "ros_car_driver"); //ROS initializes and sets the node name //ROS初始化 并设置节点名称 
...
/**************************************
Date: 2026-02-04
Function: Publish electric tracking sensor information
功能: 发布循迹传感器相关信息
***************************************/
void turn_on_robot::Publish_Ele_Sensor()
{
    std_msgs::Int16MultiArray msg;
    msg.data.resize(3);

    msg.data[0] = Ele_Sensor.Sensor_1;  // Left
    msg.data[1] = Ele_Sensor.Sensor_2;  // Center
    msg.data[2] = Ele_Sensor.Sensor_3;  // Right

    ele_sensor_publisher.publish(msg);
}
...
bool turn_on_robot::Get_Sensor_Data()
{
    ...
    uint8_t temp[64];
    int n = Stm32_Serial.read(temp, sizeof(temp));
    ...
    // 2. 只要 buffer 里可能有完整帧，就一直解析
    while (rx_buffer.size() >= RECEIVE_DATA_SIZE)
    {
        // 2.1 找帧头
        ...
        // 2.2 长度不足一帧，等下次
        if (rx_buffer.size() < RECEIVE_DATA_SIZE)
            return false;

        // 2.3 判断帧尾
        if (rx_buffer[RECEIVE_DATA_SIZE - 1] != FRAME_TAIL)
        {
            // 帧头是假的，丢弃一个字节重新同步
            rx_buffer.erase(rx_buffer.begin());
            continue;
        ...
        // 2.4 校验
        uint8_t check = Check_Sum(RECEIVE_DATA_SIZE - 2, READ_DATA_CHECK);

        if (check != rx_buffer[RECEIVE_DATA_SIZE - 2])
        {
            // 校验失败，丢弃帧头
            rx_buffer.erase(rx_buffer.begin());
            continue;
        ...
        // ✅ 2.5 到这里，说明是一帧完全正确的数据
        const uint8_t *rx = rx_buffer.data();
        Receive_Data.Flag_Stop = rx[1];

        Robot_Vel.X = Odom_Trans(rx[2], rx[3]);
        Robot_Vel.Y = Odom_Trans(rx[4], rx[5]);
        Robot_Vel.Z = Odom_Trans(rx[6], rx[7]);

        Mpu6050_Data.accele_x_data = IMU_Trans(rx[8],  rx[9]);
        Mpu6050_Data.accele_y_data = IMU_Trans(rx[10], rx[11]);
        Mpu6050_Data.accele_z_data = IMU_Trans(rx[12], rx[13]);

        Mpu6050_Data.gyros_x_data  = IMU_Trans(rx[14], rx[15]);
        Mpu6050_Data.gyros_y_data  = IMU_Trans(rx[16], rx[17]);
        Mpu6050_Data.gyros_z_data  = IMU_Trans(rx[18], rx[19]);
        ...
        transition_16  = rx[20] << 8;
        transition_16 |= rx[21];
        Power_voltage  = transition_16 / 1000.0;

        //获取循迹传感器1数据
        transition_16 = 0;
        transition_16 |=  rx[22]<<8;
        transition_16 |=  rx[23]; 
        Ele_Sensor.Sensor_1 = transition_16; 

        //获取循迹传感器2数据
        transition_16 = 0;
        transition_16 |=  rx[24]<<8;
        transition_16 |=  rx[25]; 
        Ele_Sensor.Sensor_2 = transition_16;         

        //获取循迹传感器3数据
        transition_16 = 0;
        transition_16 |=  rx[26]<<8;
        transition_16 |=  rx[27]; 
        Ele_Sensor.Sensor_3 = transition_16;         

        // 2.6 移除已解析帧
        rx_buffer.erase(rx_buffer.begin(),
                         rx_buffer.begin() + RECEIVE_DATA_SIZE);
  ...
  if(count == RECEIVE_DATA_SIZE) //Verify the length of the packet //验证数据包的长度
  {
    ...
    if(Receive_Data.Frame_Tail == FRAME_TAIL) //Verify the frame tail of the packet //验证数据包的帧尾
    {
      ...
      if(error == 0)
      {
        ...
        "%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-"
        "%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X",
        Receive_Data.rx[0],  Receive_Data.rx[1],  Receive_Data.rx[2],  Receive_Data.rx[3],
        Receive_Data.rx[4],  Receive_Data.rx[5],  Receive_Data.rx[6],  Receive_Data.rx[7],
        Receive_Data.rx[8],  Receive_Data.rx[9],  Receive_Data.rx[10], Receive_Data.rx[11],
        Receive_Data.rx[12], Receive_Data.rx[13], Receive_Data.rx[14], Receive_Data.rx[15],
        Receive_Data.rx[16], Receive_Data.rx[17], Receive_Data.rx[18], Receive_Data.rx[19],
        Receive_Data.rx[20], Receive_Data.rx[21], Receive_Data.rx[22], Receive_Data.rx[23],
        Receive_Data.rx[24], Receive_Data.rx[25], Receive_Data.rx[26], Receive_Data.rx[27],
        Receive_Data.rx[28], Receive_Data.rx[29]
        );
    */
        ...
        //获取循迹传感器1数据
        transition_16 = 0;
        transition_16 |=  Receive_Data.rx[22]<<8;
        transition_16 |=  Receive_Data.rx[23]; 
        Ele_Sensor.Sensor_1 = transition_16; 

        //获取循迹传感器2数据
        transition_16 = 0;
        transition_16 |=  Receive_Data.rx[24]<<8;
        transition_16 |=  Receive_Data.rx[25]; 
        Ele_Sensor.Sensor_2 = transition_16;         

        //获取循迹传感器3数据
        transition_16 = 0;
        transition_16 |=  Receive_Data.rx[26]<<8;
        transition_16 |=  Receive_Data.rx[27]; 
        Ele_Sensor.Sensor_3 = transition_16;         
...
void turn_on_robot::Control()
{
  ...
  while(ros::ok())
  {
                                   ...
                                   //通过串口读取并校验下位机发送过来的数据，然后数据转换为国际单位
    {
      ...
      Publish_Ele_Sensor(); //Pub the topic of electric tracking sensor //发布循迹传感器话题
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  private_nh.param<std::string>("usart_port_name",  usart_port_name,  "/dev/car_driver_tty"); //Fixed serial port number //固定串口号
  ...
  ele_sensor_publisher = n.advertise<std_msgs::Int16MultiArray>("/ele_sensor", 10);
  ...
  {
    ROS_ERROR_STREAM("ros_car_driver can not open serial port,Please check the serial port cable! "); //If opening the serial port fails, an error message is printed //如果开启串口失败，打印错误信息
  }
  if(Stm32_Serial.isOpen())
  {
    ROS_INFO_STREAM("ros_car_driver serial port opened"); //Serial port opened successfully //串口开启成功提示
  }
}
/**************************************
Date: January 28, 2021

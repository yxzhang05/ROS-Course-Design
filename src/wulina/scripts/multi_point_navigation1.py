...
# ==============================================================================
WAYPOINTS = [
    {'name': '航点1',  'x':  1.940, 'y':  0.684, 'yaw':  0.022},##第一个点执行
    {'name': '航点2',  'x':  4.380, 'y':  1.070, 'yaw':  -0.032},##直行
    {'name': '航点3',  'x':  5.830, 'y':  0.219, 'yaw':  -1.500},##第一个拐弯后
    {'name': '航点4',  'x':  5.900, 'y':  -0.808, 'yaw':  -1.500}##停下识别第一个红绿灯,
    {'name': '航点5',  'x':  4.610, 'y':  -1.990, 'yaw':  -3.000},##转向后
    {'name': '航点6',  'x':  3.850, 'y':  -1.970, 'yaw':  -3.000},##停下识别第二个红绿灯
    {'name': '航点7',  'x':  3.280, 'y':  -1.690, 'yaw':  1.500},##转向后
    {'name': '航点8',  'x':  3.420, 'y':  -0.093, 'yaw':  1.500},##直走开出去
    {'name': '航点9',  'x':  3.360, 'y':  0.428, 'yaw':  -3.000},##转向面对起点
    {'name': '航点10', 'x': 0.559, 'y': 0.230, 'yaw': 0.022},##回到终点、原地旋转
    {'name': '航点11', 'x': 5.100, 'y': -2.000, 'yaw': -1.530},
    {'name': '航点12', 'x': 5.000, 'y': -2.200, 'yaw': -2.000},
    {'name': '航点13', 'x': 4.500, 'y': -2.300, 'yaw': -3.000},
    {'name': '航点14', 'x': 4.000, 'y': -2.400, 'yaw': -2.800},
    {'name': '航点15', 'x': 3.000, 'y': -2.500, 'yaw': -3.000},
    {'name': '航点16', 'x': 2.800, 'y': -2.500, 'yaw': -3.000},
    {'name': '航点17', 'x': 3.000, 'y': -1.500, 'yaw': 1.500},
    {'name': '航点18', 'x': 1.500, 'y': 0.100, 'yaw': 2.000},
    {'name': '航点19', 'x': 0.000, 'y': 0.000, 'yaw': 3.000},
...
def main():
    ...
    # ==================== 新增：暂停控制相关变量 ====================
    # 使用字典包装可变变量，方便在回调函数中修改
    pause_state = {'paused': False}
    
    # 创建发布器，发布导航状态
    status_pub = rospy.Publisher('/navigation_status', String, queue_size=1)
    
    def pause_callback(msg):
        """接收暂停/恢复控制信号"""
        pause_state['paused'] = msg.data
        if msg.data:
            rospy.logwarn("收到暂停信号，导航暂停")
        else:
            rospy.loginfo("收到恢复信号，导航继续")
    
    # 订阅暂停控制话题
    rospy.Subscriber('/navigation_pause', Bool, pause_callback)
    # =============================================================
    ...
    # 发布状态函数
    def publish_status(status, current_idx=None, total=None, wp_name=""):
        if current_idx is not None and total is not None:
            status_msg = f"{status}|{current_idx}/{total}|{wp_name}"
        else:
            status_msg = status
    ...
    while not rospy.is_shutdown():
        ...
        # 执行导航，支持暂停恢复
        while current_waypoint_index < len(WAYPOINTS) and not rospy.is_shutdown():
            # 检查暂停状态
            while pause_state['paused'] and not rospy.is_shutdown():
                publish_status("PAUSED", current_waypoint_index + 1, len(WAYPOINTS), 
                               WAYPOINTS[current_waypoint_index].get('name', ''))
                rospy.sleep(0.5)
            ...
            # 获取当前航点
            current_wp = dict(WAYPOINTS[current_waypoint_index])
            current_wp_name = current_wp.get('name', f'航点{current_waypoint_index + 1}')
            
            # 发布当前状态
            publish_status("NAVIGATING", current_waypoint_index + 1, len(WAYPOINTS), current_wp_name)
            
            rospy.loginfo('─' * 50)
            rospy.loginfo('正在前往第 %d/%d 个航点：%s', current_waypoint_index + 1, len(WAYPOINTS), current_wp_name)
            rospy.loginfo('  目标坐标（导航坐标系）：x=%.4f  y=%.4f  yaw=%.4f rad',
                          current_wp['x'], current_wp['y'], current_wp['yaw'])
            
            # 将航点从导航坐标系变换到地图坐标系
            wp_map = _transform_waypoint(current_wp, map_rotation_offset)
            if map_rotation_offset != 0.0:
                rospy.loginfo('  目标坐标（地图坐标系）：x=%.4f  y=%.4f  yaw=%.4f rad',
                              wp_map['x'], wp_map['y'], wp_map['yaw'])
            ...
            # 获取当前位置用于判断是否需要原地旋转
            try:
                (trans, _) = tf_listener.lookupTransform(map_frame, base_frame, rospy.Time(0))
                current_x, current_y = trans[0], trans[1]
            except:
                if current_waypoint_index > 0:
                    last_wp_map = _transform_waypoint(WAYPOINTS[current_waypoint_index - 1], map_rotation_offset)
                    current_x, current_y = last_wp_map['x'], last_wp_map['y']
                else:
                    current_x, current_y = initial_pose_x, initial_pose_y
            
            xy_dist = math.hypot(wp_map['x'] - current_x, wp_map['y'] - current_y)
            
            # 判断是否使用原地旋转
            use_rotate = (xy_dist < rotate_threshold and cmd_vel_pub is not None and tf_listener is not None)
            ...
            if use_rotate:
                ...
                ok = _rotate_in_place(
                    ...
                    timeout=goal_timeout)
                if not ok:
                    rospy.logwarn('✗ 航点 [%s] 原地旋转失败或超时。', current_wp_name)
                    success = False
                    break
            else:
                # 构造并发送 move_base 目标
                ...
                while not finished and not rospy.is_shutdown():
                    # 检查是否被暂停
                    if pause_state['paused']:
                        rospy.logwarn("导航被暂停，取消当前目标")
                        client.cancel_goal()
                        # 等待恢复信号
                        while pause_state['paused'] and not rospy.is_shutdown():
                            publish_status("PAUSED", current_waypoint_index + 1, len(WAYPOINTS), current_wp_name)
                            rospy.sleep(0.5)
                        # 恢复后重新发送目标
                        if not rospy.is_shutdown():
                            rospy.loginfo("恢复导航，重新发送目标")
                            client.send_goal(goal)
                            start_time = rospy.Time.now()
                            continue
                    
                    # 检查超时
                    if (rospy.Time.now() - start_time).to_sec() > goal_timeout:
                        rospy.logwarn('航点 [%s] 导航超时（%.1f 秒），取消当前目标。',
                                      current_wp_name, goal_timeout)
                        client.cancel_goal()
                        finished = True
                        success = False
                        break
                    ...
                    # 等待结果
                    finished = client.wait_for_result(rospy.Duration(0.1))
                ...
                state = client.get_state()
                if state == actionlib.GoalStatus.SUCCEEDED:
                    rospy.loginfo('✓ 成功到达航点 [%s]', current_wp_name)
                else:
                    rospy.logwarn('✗ 未能到达航点 [%s]，状态码：%d', current_wp_name, state)
                    success = False
                    break
            ...
            # 交互模式：询问下一个航点
            next_idx = current_waypoint_index + 1
            if next_idx < len(WAYPOINTS) and interactive and not pause_state['paused']:
                publish_status("WAITING_INPUT", next_idx + 1, len(WAYPOINTS),
                               WAYPOINTS[next_idx].get('name', ''))
                next_wp, should_continue = interactive_prompt(
                    dict(WAYPOINTS[next_idx]), next_idx + 1, len(WAYPOINTS))
                if not should_continue:
                    rospy.loginfo('用户选择停止，导航提前结束。')
                    success = False
                    break
        ...
        # 发布完成状态
        if success:
            publish_status("COMPLETED", len(WAYPOINTS), len(WAYPOINTS), "全部航点")
        else:
            publish_status("FAILED", current_waypoint_index + 1, len(WAYPOINTS), "")

  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::orbbec_camera::IMUInfo_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.bias.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.bias[i]);
    }
    if (v.bias.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "scale_misalignment: ";
    if (v.scale_misalignment.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.scale_misalignment.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.scale_misalignment[i]);
    }
    if (v.scale_misalignment.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "temperature_slope: ";
    if (v.temperature_slope.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.temperature_slope.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.temperature_slope[i]);
    }
    if (v.temperature_slope.empty() || true)
      s << "]";
  }
